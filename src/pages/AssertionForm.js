...
class AssertionForm extends React.Component {
    ...
    loaded = (mappingViews) => {
        ...
        for(var i = 0; i < numElements ; i++){
            ...
            retDict.push({text: mappingViews[i].sqlViewID , displayText: mappingViews[i].sqlViewID});
        }
    ...
    onChange = (value)=>{
        ...
        function errorCheckElement(string , numElements){
            var temp =  (string.match(/,/g) || []).length;
            if(temp > numElements){
                alert("Error");
            }
        }
            ...
            for(var i = 0; i < numAttributes; i++){
                for(var j = 0; j < numViews ; j++){
                    if(this.state.mappingViews.filter(v => v.sqlViewID === views[j].trim())[0]){
                        let temp = optimizeMappingBody(this.state.mappingViews.filter(v => v.sqlViewID === views[j].trim())[0].sqlViewCode.toLowerCase() , "view")[0].split("select ")[1];
