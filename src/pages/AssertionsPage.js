    ...
    if (data.length === 0) {
      ...
      this._isMounted &&
        this.setState({
          data: data,
          loading: false,
          currentEntity: data[0].currentEntity
        });
  };

  loadedEntity = entity => {
    this._isMounted && this.setState({ loading: false, currentEntity: entity });
  };
    ...
    return (
      <div>
        <div style={{ textAlign: "center" }}>
          <h1>
            {this.state.data.length} mappings for{" "}
            <Entity
          ...
          <Popover
            content={this.state.currentEntity.entityIRI}
            placement="bottom"
          >
