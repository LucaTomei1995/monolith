...
public class TestKGUnloader {
	...
	public static void main(String[] args) throws IOException, InterruptedException, OWLOntologyCreationException, OWLOntologyStorageException, MappingFileMalformedException, SAXException {
		...
		List<String> removeFileNames = new LinkedList<String>();
		removeFileNames.add("Regione_18.ntriples");
		
		System.out.println(" --------------- REMOVING TRIPLES FILES -----------------");
		
		KnowledgeGraphFileDestination fileDestination2 = new KnowledgeGraphFileDestination();
		fileDestination2.setFileNames(removeFileNames);
		fileDestination2.setKgDestination(kgDestination);
		manager.putKGUploadRemove(session, fileDestination2);
