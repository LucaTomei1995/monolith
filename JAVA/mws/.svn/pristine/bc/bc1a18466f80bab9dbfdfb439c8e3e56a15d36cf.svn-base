...
public class TestKGMentionedClasses2 {
	...
	public static void main(String[] args) throws IOException, InterruptedException, OWLOntologyCreationException, OWLOntologyStorageException, MappingFileMalformedException, SAXException {
		...
		kgf.setFile(Utils.getFileInfo("/home/marco/Scaricati/enti_territoriali.nt", "enti_territoriali.nt", "N3"));
		...
		System.out.println(om.writeValueAsString(named.iterator().next()));

		Set<String> mentioned = manager.getKGMentionedClasses(kgIri, "http://kg/iris/com/ui/q1");
		System.out.println(om.writeValueAsString("Named graphs: " + mentioned));
		
		Set<String> mentionedInstances = manager.getKGMentionedInstances(kgIri, "http://kg/iris/com/ui/q1");
		System.out.println("Total instances: " + mentionedInstances.size());
		System.out.println("Instance sample: " + mentionedInstances.iterator().next());
		
		Set<String> instancesByClass = manager.getKGInstancesForClass(kgIri, "http://kg/iris/com/ui/q1", "http://www.aci.it/ontology/Provincia");
		System.out.println("Instances for Provincia: " + instancesByClass.size());
		System.out.println("Sample instance: " + instancesByClass.iterator().next());
		
		InstancePage ip = manager.getInstancePage(session, kg, "http://www.aci.it/ontology/Provincia/079", "http://kg/iris/com/ui/q1");
		System.out.println(om.writeValueAsString(ip));
