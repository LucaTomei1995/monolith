...
public class TestUnit1Query {
	...
	public static void main(String[] args) throws IOException, InterruptedException, OWLOntologyCreationException, OWLOntologyStorageException, MappingFileMalformedException, SAXException {
		...
		String code = new String(Files.readAllBytes(Paths.get("./src/test/resources/specs/unit1/queries/q02.sparql")));
		query.setQueryCode(code);
		manager.postSPARQLQuery(session, "UNIT1", "http://www.obdasystems.com/unit1/1.0", query);
		
		// starting mastro instance
		MastroID id = new MastroID();
		OntologyID oid = new OntologyID();
		oid.setOntologyName("UNIT1");
		oid.setOntologyVersion("http://www.obdasystems.com/unit1/1.0");
		id.setMappingID("UNIT1_01");
		...
		OBDARunQueryInstance oqri = manager.startQueryFromCatalog(session, "UNIT1", "http://www.obdasystems.com/unit1/1.0", "UNIT1_01", "q01", true);
		System.out.println(om.writeValueAsString(oqri));
		
		Thread.sleep(3000);
		
		SPARQLStatus qstatus = manager.getQueryStatus(session, "UNIT1", "http://www.obdasystems.com/unit1/1.0", "UNIT1_01", oqri.getExecutionId());
		System.out.println(om.writeValueAsString(qstatus));
		
		
		Thread.sleep(3000);
		
		qstatus = manager.getQueryStatus(session, "UNIT1", "http://www.obdasystems.com/unit1/1.0", "UNIT1_01", oqri.getExecutionId());
		System.out.println(om.writeValueAsString(qstatus));
