...
public class KGMultiFilesLoader implements Runnable {
	...
	public KGMultiFilesLoader(List<KnowledgeGraphFile> _kgds, List<String> _realNames, String _kgFolder, String _tdbFolder, String _modelFolderPath) {
		this.kgds = _kgds;
	...
	public void run() {
		...
		try {
			...
			for (int i=0; i < this.kgds.size(); i++) {
				...
				for (KnowledgeGraphFileInfo info : infos.getInfos()) {
					if (info.getFileName().equals(actRealName)) {
						...
						info.setImportingTime(loadingTime);
						info.setNumberOfTriples(count);
			...
			try {
				Files.write(semReady, new String("Triples loaded: " + total).getBytes());
			} catch (IOException e) {
